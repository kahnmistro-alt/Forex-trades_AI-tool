...
class PatternModel:
    ...
    def prepare_features(self, df):
        """Compute features and return (X, positions) where positions are integer row indices of clean data."""
        if len(df) < 20:
            return None, None
        df = df.copy()
        ...
        return X, df_clean.index  # df_clean.index are integer positions

    def train(self, df, labels=None):
        X, idx = self.prepare_features(df)
        if X is None:
            return False
        # idx are integer positions; use them to slice df for clean rows
