...
ERROR_DICT['02102'] = 'Failed to copy ticks'
ERROR_DICT['02103'] = 'Not imlemented in MT4'

ERROR_DICT['02201'] = 'Instrument not in demo'
ERROR_DICT['02202'] = 'Failed to copy ticks'
ERROR_DICT['02203'] = 'Not imlemented in MT4'

ERROR_DICT['02301'] = 'Instrument not in demo'
ERROR_DICT['02302'] = 'Unknown instrument for broker'
ERROR_DICT['02303'] = 'Unknown instrument for broker'
ERROR_DICT['02304'] = 'Time out error'
...
ERROR_DICT['07502'] = 'Wrong parameter amount' 
ERROR_DICT['07503'] = 'Error in modify'
ERROR_DICT['07504'] = 'Position does not exist' 
...
class Pytrader_API:             
    ...
    def __init__(self):
        ...
        self.socket_data_size: int = 10000000
        ...
        self.date_from: datetime = datetime(2024, 1, 1, 0, 0, 0)
    ...
    def Check_trading_allowed(self,
        ...
        """
        # Set the universal instrument name first
    ...
    def get_PnL_SoD(self, offset: int = 3600) -> pd.DataFrame:
        """
        Calculate Profit and Loss (PnL) for the start of day (SoD).
        
        This method computes realized and unrealized profit/loss for positions based on
        the broker's server time. It uses the previous day's close price as the baseline
        for trades opened before today, and the trade's open price for trades opened today.
        
        The calculation handles four scenarios:
        1. Trade opened before today and closed today: Uses previous day's close as baseline
        2. Trade opened today and closed today: Uses trade's open price as baseline
        3. Trade opened before today and still open: Uses previous day's close as baseline
        4. Trade opened today and still open: Uses trade's open price as baseline
        
        All PnL calculations include swap and commission.
        
        Args:
            offset: Time offset in seconds (default: 3600). 
        
        Returns:
            pd.DataFrame: A DataFrame with a single row containing:
                - SoD_profit: Total profit/loss for the day (realized + unrealized)
                - realized_profit: Total realized profit/loss from closed positions
                - unrealized_profit: Total unrealized profit/loss from open positions
                
            Returns empty DataFrame if broker server time cannot be retrieved.
        
        Note:
            - Uses Get_broker_server_time() to ensure time consistency with the platform
            - Day start is calculated as midnight (00:00:00) of the current server day
            - Closed positions are filtered to include only those closed on or after day_start
            - Previous day's close price is fetched using Get_last_x_bars_from_now(nbrofbars=2)
        """
        total_realized = 0.0
        ...
        server_time = self.Get_broker_server_time()
        if server_time is None:
            return pd.DataFrame()
        
        now = server_time
        day_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
        now_ts = int(now.timestamp()) + offset* 1000
        ...
        def _get_prev_close(universal_instrument: str):
            if universal_instrument in prev_close_cache:
                return prev_close_cache[universal_instrument]

            bars = self.Get_last_x_bars_from_now(
                instrument=universal_instrument,
                timeframe=self.get_timeframe_value("D1"),
                nbrofbars=2,
            )
            if bars is None or len(bars) < 2:
                prev_close_cache[universal_instrument] = None
                return None

            bars_sorted = np.sort(bars, order="date")
            prev_close = float(bars_sorted[-2]["close"])
            prev_close_cache[universal_instrument] = prev_close
            return prev_close

        def _get_instr_info(universal_instrument: str):
            if universal_instrument in instr_info_cache:
                return instr_info_cache[universal_instrument]
            info = self.Get_instrument_info(universal_instrument)
            instr_info_cache[universal_instrument] = info
            return info

        def _pnl_from_price_diff(universal_instrument: str, price_diff: float, volume: float):
            info = _get_instr_info(universal_instrument)
            if not info:
                return None
            ...
            if tick_size <= 0.0 or tick_value == 0.0:
                ...
                if point <= 0.0 or tick_value == 0.0:
                    return None
                tick_size = point
            return (price_diff / tick_size) * tick_value * float(volume)

        def _get_quote(universal_instrument: str):
            if universal_instrument in quote_cache:
                return quote_cache[universal_instrument]
            quote = self.Get_last_ask_bid(universal_instrument)
            quote_cache[universal_instrument] = quote
            return quote

        if type(closed_positions) == pd.DataFrame and not closed_positions.empty:
            if "close_time" in closed_positions.columns:
                closed_positions_filtered = closed_positions[
                    (closed_positions["close_time"] >= day_start_ts)
                    & (closed_positions["close_time"] <= now_ts)
                ]

            for position in closed_positions_filtered.itertuples():
                instrument = str(position.instrument)
                opened_today = int(position.open_time) >= day_start_ts
                if opened_today:
                    baseline_price = float(position.open_price)
                else:
                    prev_close = _get_prev_close(instrument)
                ...
                if position.position_type == "buy":
                    price_diff = close_price - baseline_price
                elif position.position_type == "sell":
                    price_diff = baseline_price - close_price
                else:
                    continue

                pnl = _pnl_from_price_diff(instrument, price_diff, float(position.volume))
                if pnl is None:
                    continue

                pnl += float(position.commission) + float(position.swap)
                ...
                if pnl > 0.0:
                    trades_in_profit += 1
                    volume_in_profit += float(position.volume)
                else:
                    trades_in_loss += 1
                    volume_in_loss += float(position.volume)

        if type(open_positions) == pd.DataFrame and not open_positions.empty:
            for position in open_positions.itertuples():
                instrument = str(position.instrument)
                opened_today = int(position.open_time) >= day_start_ts
                if opened_today:
                    baseline_price = float(position.open_price)
                ...
                if position.position_type == "buy":
                    ...
                    price_diff = baseline_price - current_price
                else:
                    continue
        ...
        result = {
            "SoD_profit": float(total_realized + total_unrealized),
    ...
    def Get_ticks_in_range(self,
                            instrument: str = 'EURUSD',
                            date_from: datetime = (pytz.timezone("US/Eastern").localize(datetime.now()) - timedelta(days=2)),
                            date_to: datetime = pytz.timezone("US/Eastern").localize(datetime.now()),
                            ) -> np.ndarray:
        ...
        if (date_from >= date_to):
            self.command_return_error = 'date_from is after or equal to date_to'
            self.command_OK = False
            return None

        dt = np.dtype([('date', np.int64), ('ask', np.float64), ('bid', np.float64), ('last', np.float64), ('volume', np.int32)])

        self.command = 'F022^5^' + self.instrument + '^' + self.date_from.strftime('%Y/%m/%d/%H/%M/%S') + '^' + self.date_to.strftime('%Y/%m/%d/%H/%M/%S') + '^'
    ...
    def Get_last_ask_bid(self,
        ...
        if (self.instrument == 'none' or self.instrument == None):
            ...
            self.command_OK = False
            return None
        ok, dataString = self.send_command('F023^2^' + self.instrument + '^')
        ...
        # Populate the dictionary with the tick data
        returnDict['instrument'] = str(self.instrument_name_universal)
        returnDict['ask'] = float(x[0])
        returnDict['bid'] = float(x[1])
    ...
    def Get_actual_bar_info(self,
        ...
        Args:
            instrument (str): instrument name
            timeframe (int): time frame like H1, H4
    ...
    def Get_bars_from_date_to_date(self, 
                                    instrument: str = 'EURUSD',
                                    timeframe: int = 16408,
                                    from_date: datetime = datetime(2021, 3, 1, tzinfo = pytz.timezone("Etc/UTC")),
                                    to_date: datetime = datetime.now()) -> np.array:
        """
        ...
        if (from_date >= to_date):
            self.command_return_error = 'From date is behind to date'
            self.command_OK = False
            return None
        ...
        # Normalize dates to 00:00:00 as required and convert to UTC epoch seconds
        def _normalize_to_utc_epoch(d: datetime) -> int:
            # set to 00:00:00 first
            d0 = d.replace(hour=0, minute=0, second=0, microsecond=0)
            # timezone handling: if naive, assume UTC; else convert to UTC
            if d0.tzinfo is None:
                d0 = pytz.timezone("Etc/UTC").localize(d0)
            else:
                d0 = d0.astimezone(pytz.timezone("Etc/UTC"))
            return int(d0.timestamp())

        from_epoch = _normalize_to_utc_epoch(from_date)
        to_epoch = _normalize_to_utc_epoch(to_date)

        # Validate timeframe by converting to minutes (no direct use for count here)
        tf_minutes = self.get_timeframe_in_minutes(timeframe)
        if tf_minutes <= 0:
            self.command_return_error = ERROR_DICT['04302']
            self.command_OK = False
            return None

        # Iterate day by day, requesting by date window using F043^6^
        out_rows = []
        utc = pytz.timezone("Etc/UTC")
        cur_start = datetime.fromtimestamp(from_epoch, tz=utc)
        hard_end = datetime.fromtimestamp(to_epoch, tz=utc)

        while cur_start <= hard_end:
            next_start = cur_start + timedelta(days=1)
            # The chunk end is the lesser of next day or the hard end
            chunk_end = hard_end if next_start > hard_end else (next_start - timedelta(seconds=1))

            # Set dates on self for consistency with other methods
            self.date_from = cur_start
            self.date_to = chunk_end

            self.command = (
                'F043^6^' + self.instrument + '^' + str(timeframe) + '^' +
                self.date_from.strftime('%Y/%m/%d/%H/%M/%S') + '^' +
                self.date_to.strftime('%Y/%m/%d/%H/%M/%S') + '^'
            )
            ok, dataString = self.send_command(self.command)
            if not ok:
                self.command_OK = False
            ...
            if len(x) > 0:
                for value in range(len(x)):
                    y = x[value].split('$')
                    out_rows.append((int(y[0]), float(y[1]), float(y[2]), float(y[3]), float(y[4]), int(y[5])))

            # advance to next day
            cur_start = next_start

        # If nothing retrieved, return empty array
        if len(out_rows) == 0:
            self.command_OK = True
            return np.zeros(0, dtype=dt)

        # Build numpy array
        rates = np.array(out_rows, dtype=dt)

        # Sort and filter strictly within [from_epoch, to_epoch]
        rates = np.sort(rates[rates[:]['date'] != 0])
        mask = (rates['date'] >= from_epoch) & (rates['date'] <= to_epoch)
        rates = rates[mask]
    ...
    def Get_deleted_orders_within_window(self,
        ...
        for value in range(len(x)):
            ...
            rowOrder = {
                ...
                'delete_price': float(y[10]),
                'delete_time': int(y[11]),
                'comment': str(y[12])
            }
    ...
    def Get_all_open_positions(self) -> pd.DataFrame:
        """
        Retrieves all open positions, market orders for MT4/5.
    ...
    def Open_order(self,
        ...
        if (broker_instrument == 'none' or broker_instrument == None):
    ...
    def Close_positions_async(self,
        ...
        This function will close a position by instrument and magic number.
    ...
    def send_command(self,
        ...
        try:
            ...
            data_received = ''
            while True:
                data_received = data_received + self.sock.recv(self.socket_data_size).decode()
    ...
    def get_timeframe_in_minutes(self,
                                 timeframe: int = 60) -> int:
        """
        Convert a timeframe value to minutes.

        Args:
            int: The timeframe in MT5 int values

        Returns:
            int: The timeframe in minutes.
        """
        self.tf_in_minutes = -1
        if timeframe == 49153:
            self.tf_in_minutes = 43200
        elif timeframe == 32769:
            self.tf_in_minutes = 10080
        elif timeframe == 16408:
            self.tf_in_minutes = 1440
        elif timeframe == 16396:
            self.tf_in_minutes = 720
        elif timeframe == 16392:
            self.tf_in_minutes = 480
        elif timeframe == 16390:
            self.tf_in_minutes = 360
        elif timeframe == 16388:
            self.tf_in_minutes = 240
        elif timeframe == 16387:
            self.tf_in_minutes = 180
        elif timeframe == 16386:
            self.tf_in_minutes = 120
        elif timeframe == 16385:
            self.tf_in_minutes = 60
        elif timeframe == 30:
            self.tf_in_minutes = 30
        elif timeframe == 20:
            self.tf_in_minutes = 20
        elif timeframe == 15:
            self.tf_in_minutes = 15
        elif timeframe == 12:
            self.tf_in_minutes = 12
        elif timeframe == 10:
            self.tf_in_minutes = 10
        elif timeframe == 6:
            self.tf_in_minutes = 6
        elif timeframe == 5:
            self.tf_in_minutes = 5
        elif timeframe == 4:
            self.tf_in_minutes = 4
        elif timeframe == 3:
            self.tf_in_minutes = 3
        elif timeframe == 2:
            self.tf_in_minutes = 2
        elif timeframe == 1:
            self.tf_in_minutes = 1

        return self.tf_in_minutes
